...
- `@code-reviewer` : runs once per non-trivial slice. Runs the quality gate and reviews; diffs the verification surface Red -> Green (see `<test-discipline>`). Has a `Stop` hook that re-runs the gate and blocks completion on failure.{{CODEX_ROSTER_NOTE}}
...
For trivial tasks as defined in `<exceptional-cases>` (no behavioral effect): skip subagents entirely. Make the change directly, run `{{QA_COMMAND}}`, commit.
</agent-roster>
...
- **A goal condition is a driver, not a verifier.** Phrase goal conditions in gate terms -- "`{{QA_COMMAND}}` green, every AC's named test passing, ship record valid, or stop after N turns" -- and remember that a goal evaluator only reads what the session surfaced; the deterministic hooks, the gate command, and CI remain the authority on "done". A loop that cannot cite a gate run has not finished.
