...
#
# Fails if any *.go file in the project exceeds 200 lines. The ~100-line target
...
# e.g. `internal/pb/service.pb.go`). vendor/ and .claude/ are always skipped.
...
list_files() {
  if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    git ls-files -- '*.go' | grep -Ev '^(vendor|\.claude)/' || true
  else
    find . -name '*.go' -type f -not -path './vendor/*' -not -path './.claude/*' 2>/dev/null | sed 's|^\./||'
  fi
}
