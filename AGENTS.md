...
- **Conditional content inside always-on files.** Some always-on files carry AI-only sections fenced with HTML-comment markers; `SKILL.md` Phase 4 rule 5 keeps or strips them by Q6. Current fences: `<!-- AI-SECURITY-START/END -->` + `<!-- AI-REDTEAM-START/END -->` in `docs/SECURITY.md`, `<!-- AI-FEATURES-START/END -->` in `docs/requirements.md`, `<!-- AI-IMPL-START/END -->` in `.claude/agents/implementer.md`, `<!-- AI-REVIEW-START/END -->` in `.claude/agents/code-reviewer.md`. Fence any future sometimes-on content the same way, and register the new marker in Phase 4 rule 5 AND in `upgrade-project/SKILL.md` Phase 3-A.
