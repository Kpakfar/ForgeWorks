...
#   - Python + Go `scripts/linecap.sh`: green on the shipped scaffold, red on a
#     synthetic 201-line file, green at exactly 200 lines, green again when the
#     offender is allowlisted in .linecap-ignore.
#   - Exercises BOTH file-listing paths: git ls-files (Go fixture is a git repo)
#     and the find fallback (Python fixture is a bare directory).
