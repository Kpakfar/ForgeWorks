...
0. **Static checks**: the `Stop` hook runs `npm run qa` when your review completes and blocks completion on failure -- do not run the full gate yourself as a separate pass. Re-run only the specific step you are investigating (a single failing test, one lint rule). All steps must pass for the review to complete.
...
- Never APPROVE with the gate red: your Stop hook runs `npm run qa` and blocks on failure. The gate is the gate.
