...
- The bundled quality-gate command is `npm run qa` (runs lint + format + types + unit/functional tests in order). It is wired into the QA hook and the CI workflow. Do not bypass it. End-to-end tests run separately (slower) in CI; see `<test-discipline>`.
</development-process>
...
Mocks only for external services you do not own; prefer recorded responses. The inner loop stays fast: `npm run qa` runs lint/format/types plus unit and functional tests. The slower **headless-browser e2e** suite runs in CI and pre-merge, not on every TDD cycle.
</test-discipline>
<!-- /FW-BLOCK: test-discipline -->

<!-- FW-BLOCK: style-references v2.0.0 -->
<style-references>
Pattern-match every file you write or modify to the clean SPA structure of excalidraw/excalidraw. Reference material: https://github.com/excalidraw/excalidraw.
...
**How to use it**:
- Before writing the code, query Context7 for the relevant API of the **pinned version** in your manifest file (package.json), not the latest available.
...
- Use the project's lint/format/type/test toolchain (recorded in `docs/language-standards.md`). The `npm run qa` script chains all of them.
...
1. **Static + test hook.** Before declaring any task complete, run `npm run qa` -- it **verifies only and changes no files** (lint, format *check*, type-check, then unit + functional tests, in order). All must pass. If a step fails, fix the cause; to auto-repair formatting/lint locally run `npm run fix`, review the diff, then commit. Don't skip steps. Don't comment out failing tests. The `code-reviewer`'s `Stop` hook (auto-converted to `SubagentStop`) runs `npm run qa` when the review completes and blocks completion (exit code 2) on failure, so APPROVE cannot ship a red build; the reviewer itself re-runs only the specific failing step it is investigating, not the whole gate. Because the gate never mutates code, it cannot silently "fix" and pass.
...
3. **CI.** CI runs the same non-mutating `npm run qa` plus the slower end-to-end (headless-browser) suite (see `.github/workflows/qa.yml`). Note: the shipped workflow runs on pull requests and on pushes to `main`; **merge-blocking requires enabling branch protection** on the repo (the template cannot set that for you).
...
For trivial tasks as defined in `<exceptional-cases>` (no behavioral effect): skip subagents entirely. Make the change directly, run `npm run qa`, commit.
</agent-roster>
...
- **A goal condition is a driver, not a verifier.** Phrase goal conditions in gate terms -- "`npm run qa` green, every AC's named test passing, ship record valid, or stop after N turns" -- and remember that a goal evaluator only reads what the session surfaced; the deterministic hooks, the gate command, and CI remain the authority on "done". A loop that cannot cite a gate run has not finished.
